...
import EditSupplierGroupsForm from "../../Components/EditSupplierGroupsForm";
import { useParams } from "react-router-dom";
const EditSupplierGroupsPage = () => {
    const {id}=useParams();
    ...
    useEffect(() => {
        setLoading(true);
        axios
          .get(`${baseUrl}/get/supplier/group/${id}`, {
            headers: {
          ...
          .then(function (response) {
            ...
            if (response.data != "") {
              ...
              setSupplierGroup(response.data.data)
            } else {
    ...
    <div className="w-full admin-dashboard">
      <div className="flex flex-row w-full w-full items-center p-3 justify-between">
        <section class=" text-black w-5/6 p-6 mx-auto bg-white rounded-lg shadow-md dark:bg-gray-800 mt-20">
         {dockTypes&& buildingData&& supplierGroup&& <EditSupplierGroupsForm id={id} supplierGroup={supplierGroup} buildings={buildingData} dockTypes={dockTypes}/>}
        </section>
      </div>
    </div>
  );
...
export default EditSupplierGroupsPage ;
