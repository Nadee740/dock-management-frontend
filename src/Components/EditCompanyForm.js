...
const EditCompanyForm = ({companyData}) => {
    ...
    const [company_name,setCompanyname]=useState(companyData.company_name);
    const [building_name,setSelectedBuilding]=useState('');
    ...
    const submitData=async()=>{
        ...
        const data = {
          ...
          axios.post(`${baseUrl}/edit-company/${companyData._id}`,data,{
            headers:{
                'Authorization': `Bearer ${Token}`
            }
          })
          .then((res) => {
            ...
            {
               setModalHeading("Company Updated Successfully")
               setModalText("")
            ...
            <div className="mb-2">
                ...
                <select  onChange={(e)=>{
                    ...
                    {building.map((b,index)=>{
                        return <>
                        {building_name===b._id?<option selected value={b._id}>{b.building_name}</option>:<option value={b._id}>{b.building_name}</option>}
                        </> 
                    })
